...
- Dependabot scans the flat `requirements/` pip-compile layout where source
  `.in` files and generated `.txt` files live together; the lock update
  workflow is available for manual source-first dependency updates.
...
- Updates selected requirement source files (`requirements.in` and/or
  `requirements/*.in`), regenerates all `requirements/*.txt` files with pinned
  lock tooling, and opens a pull request when the generated dependency state
  changes.
